"""
supervertaler_assistant.agents.lint
===================================

Health Check agent.

Scans the memory bank, sends a snapshot to the LLM with the ``lint.md``
...
2. Write the files using the shared :mod:`supervertaler_assistant.agents._output`
"""
...
from ..memory_bank import CONTENT_FOLDERS, iter_articles
from ._output import (
    WriteResult,
    parse_file_blocks,
    summarise_writes,
...
# Cap on how many chars of memory bank content we hand to the LLM in one
# go. At ~4 chars/token this is ~30k tokens – well within the budget of
...
class LintResult:
    ...
    def summary(self) -> str:
        ...
        if self.articles_scanned == 0:
            return "Memory bank has no content to check."
        ...
        if self.truncated:
            bits.append(
                "⚠ Memory bank exceeds single-pass snapshot limit – results "
                "may be incomplete. Consider running Process Inbox more often "
...
class LintAgent:
    """Health Check – scans the memory bank for problems and applies auto-fixes."""

    def __init__(self, llm: LlmClient, memory_bank_dir: str | Path) -> None:
        self.llm = llm
        self.memory_bank_dir = Path(memory_bank_dir).resolve()
        self._prompt = _load_lint_prompt(self.memory_bank_dir)

    # ── Public API ──────────────────────────────────────────────────────

    def run(self, *, dry_run: bool = False) -> LintResult:
        """Run one full health check pass over the whole memory bank."""
        result = LintResult()

        try:
            snapshot, count, truncated = _build_snapshot(self.memory_bank_dir)
            ...
            messages = [
                system(self._prompt),
                user(
                    "Here is a snapshot of the current memory bank. "
            ...
            if blocks:
                result.writes = write_file_blocks(
                    self.memory_bank_dir, blocks, dry_run=dry_run
                )
...
def _build_snapshot(memory_bank_dir: Path) -> tuple[str, int, bool]:
    """Concatenate all memory bank articles into one labelled Markdown blob.
    ...
    """
    articles = list(iter_articles(memory_bank_dir))
    if not articles:
        return "", 0, False
    ...
    for entry in articles:
        ...
        header = f"### MEMORY BANK FILE: {entry.relative_path}\n\n"
...
def _load_lint_prompt(memory_bank_dir: Path) -> str:
    """Load lint.md, preferring the memory bank's copy over the bundled fallback."""
    user_template = memory_bank_dir / "06_TEMPLATES" / "lint.md"
    ...
    return (
        "You are the Supervertaler Assistant in tidy-memory mode. Scan the "
        "provided memory bank snapshot and report inconsistencies, broken "
        "links, stale content, and duplicates. Apply auto-fixes via "
        "`### FILE:` blocks."
    )
