"""
supervertaler_assistant.agents.compile
======================================
...
``### FILE:`` blocks from the reply, writes them into the memory bank,
and archives the source files.
...
Running this does NOT consult the existing memory bank – Process Inbox
produces raw articles, it doesn't translate. Conflict detection is the
lint agent's job.
"""
...
class CompileAgent:
    ...
    def __init__(self, llm: LlmClient, memory_bank_dir: str | Path) -> None:
        ...
        self._prompt = _load_compile_prompt(self.memory_bank_dir)
        ...
        try:
            ...
            if not dry_run and any(w.ok for w in result.writes):
                result.inbox_files_archived = _archive_sources(
                    files, self.memory_bank_dir, blocks
                )
    ...
    def _call_llm(self, files: list[Path]) -> str:
        ...
        bundle = _bundle_inbox_files(files, self.memory_bank_dir)
        messages = [
            system(self._prompt),
            user(
...
def _load_compile_prompt(memory_bank_dir: Path) -> str:
    """Load compile.md, preferring the memory bank's copy over the bundled fallback."""
    user_template = memory_bank_dir / "06_TEMPLATES" / "compile.md"
    ...
    return (
        "You are the Supervertaler Assistant in organise-memory mode. Read "
        "the inbox material and produce structured Markdown articles using "
        "`### FILE: <path>` blocks. Use [[backlinks]] for cross-references."
    )


def _bundle_inbox_files(files: list[Path], memory_bank_dir: Path) -> str:
    ...
    for f in files:
        try:
            ...
            continue
        rel = f.relative_to(memory_bank_dir).as_posix()
...
def _archive_sources(
    ...
    compile.md.
    """
    archive_dir = memory_bank_dir / "00_INBOX" / "_archive"
