"""
supervertaler_assistant.agents._output
======================================
...
2. Validates target paths (``is_safe_memory_bank_path``) so a malicious or
   confused reply can't write outside the memory bank or into system folders.
...
# in memory_bank.py plus 00_INBOX (for agents that drop reports there).
...
def is_safe_memory_bank_path(memory_bank_dir: Path, relative_path: str) -> bool:
    """Return True if ``relative_path`` is a safe write target in the memory bank.

    Rules:
    - Must resolve to a descendant of ``memory_bank_dir`` (no ``..`` traversal).
    ...
    # Final resolution check – must stay inside memory_bank_dir
    try:
        target = (memory_bank_dir / p).resolve()
        target.relative_to(memory_bank_dir.resolve())
...
def write_file_blocks(
    ...
    *,
    ...
    ----------
    memory_bank_dir
        The root of the target memory bank.
    blocks
    ...
    for block in blocks:
        if not is_safe_memory_bank_path(memory_bank_dir, block.relative_path):
            results.append(
                WriteResult(
                    block=block,
                    ok=False,
        ...
        target = (memory_bank_dir / block.relative_path).resolve()
