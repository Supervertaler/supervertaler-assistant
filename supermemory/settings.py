...
Persisted app settings — vault path, LLM provider/model/key, UI state.

Settings live in a single JSON file at a cross-platform per-user path
computed by ``platformdirs``:

    Windows : %APPDATA%\\Supervertaler\\SuperMemory\\settings.json
    macOS   : ~/Library/Application Support/SuperMemory/settings.json
    Linux   : ~/.config/SuperMemory/settings.json
...
class AppSettings:
    ...
    # Per-session hints that flow into KB context lookups
    project_name: str = ""
    domain: str = ""
    source_lang: str = ""
    target_lang: str = ""
