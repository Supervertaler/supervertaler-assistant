...
Python port of `SuperMemoryReader.cs` from Supervertaler for Trados.

Reads and queries a SuperMemory Obsidian vault. Scans frontmatter for
lightweight indexing, then loads full articles on demand. Produces a
`KbContext` for a given client / domain / language pair that can be
formatted straight into an LLM prompt.

Design goals
------------
- 100% vault-format-compatible with the Trados plugin (same folder names,
  same frontmatter keys, same scoring rules).
...
- No LLM dependency in this module — pure I/O + parsing. The agents
  (compile / lint / query / translate) live in `supermemory.agents.*`
  and consume the objects defined here.
...
"""Folders scanned for KB articles. 00_INBOX, 05_INDICES, 06_TEMPLATES are
skipped — they are workflow folders, not knowledge."""
...
@dataclass
class KbArticleIndex:
    """Lightweight index entry for a KB article (frontmatter only)."""
...
@dataclass
class KbContext:
    """Resolved KB context for a translation — the articles loaded and ready."""
    ...
    def summary(self) -> str | None:
        ...
        if not parts:
            return None
        return f"SuperMemory: {', '.join(parts)} (~{self.estimated_tokens} tokens)"


# ─── Reader ─────────────────────────────────────────────────────────────────


class SuperMemoryReader:
    """Reads and queries a SuperMemory Obsidian vault.
    ...
    def __init__(self, vault_dir: str | Path) -> None:
        ...
        self._index: list[KbArticleIndex] | None = None
    ...
    def vault_exists(self) -> bool:
        """True if the vault has at least one expected content folder."""
        return self.vault_dir.is_dir() and any(
            (self.vault_dir / f).is_dir() for f in CONTENT_FOLDERS
        )
    ...
    def refresh_index(self, force: bool = False) -> None:
        ...
        entries: list[KbArticleIndex] = []

        for folder in CONTENT_FOLDERS:
            folder_dir = self.vault_dir / folder
    ...
    def load_context(
        ...
        *,
        ...
        """Load relevant KB context for a translation task.

        Returns None if the vault doesn't exist or has no relevant content.
        """
        ...
        client_entry: KbArticleIndex | None = None
    ...
    @staticmethod
    def format_for_prompt(ctx: KbContext | None) -> str | None:
        """Format a KbContext as a prompt-ready Markdown section.

        Mirrors SuperMemoryReader.FormatForPrompt in the C# plugin so the
        same prompts work across both.
        """
        ...
        lines: list[str] = [
            "# KNOWLEDGE BASE",
            "",
            "The following context comes from your SuperMemory knowledge base.",
            "Use this information to inform your translations and terminology choices.",
            "Knowledge base decisions take priority over general assumptions.",
        ]
    ...
    def _detect_client(self, project_name: str) -> KbArticleIndex | None:
        """Best-effort client auto-detection from a Trados / Workbench
        project name. Matches `SuperMemoryReader.DetectClient` semantics."""
    ...
    def _find_terminology_articles(
        ...
        *,
        ...
        scored: list[tuple[KbArticleIndex, int]] = []
        ...
        # If nothing scored and the vault is small, return all term articles —
...
def _extract_lang_code(lang_display: str | None) -> str:
    ...
    Matches `SuperMemoryReader.ExtractLangCode` so detection results are
    identical across the C# and Python readers.
    """
    if not lang_display:
        return ""
...
def _matches_domain(entry: KbArticleIndex, domain: str) -> bool:
...
def iter_articles(vault_dir: str | Path) -> Iterable[KbArticleIndex]:
    """Yield every indexed KB article in the vault (examples/archives skipped).

    Handy for the Health Check agent, which wants the full list without
    going through `load_context`.
    """
    reader = SuperMemoryReader(vault_dir)
