"""
supermemory.agents.query
========================

Chat backend for the SuperMemory standalone app.

This is the simplest of the four agents because it has no file-writing
side effects — the user asks a question, the agent reads the vault, and
the LLM produces an answer with ``[[backlinks]]`` citations.

Design
------
- The **system prompt** is loaded from ``06_TEMPLATES/query.md`` in the
  user's vault when available, otherwise from the bundled template
  shipped with the app (``supermemory/templates/query.md``). Users who
  edit their vault template get their edits picked up; users who don't
  always get a sensible default.

- **KB context is injected fresh on every turn** rather than pre-loaded
  and sticky. This keeps the conversation grounded as the user pivots
  between topics (e.g. "now what about client X?"), and it means that
  vault edits become visible without restarting the chat. The 30s
  frontmatter-index cache in
  :class:`supermemory.vault.SuperMemoryReader` keeps the cost low.
...
def _load_query_prompt(vault_dir: Path) -> str:
    """Load the query agent's system prompt.

    Priority:
        1. ``{vault}/06_TEMPLATES/query.md`` if the user has one
        2. bundled fallback in ``supermemory/templates/query.md``
    """
    user_template = vault_dir / "06_TEMPLATES" / "query.md"
    ...
    return (
        "You are the SuperMemory reference librarian. Answer the user's "
        "questions about their translation knowledge base, citing sources "
        "with [[backlinks]]. If the KB is silent on a topic, say so."
    )
...
class ChatTurn:
    ...
    We store both the raw user question and the KB context that was
    injected for it, so the UI can show "this turn consulted X, Y, Z" if
    the user wants to inspect provenance.
    """
...
class ChatSession:
    ...
    # Optional per-session context hints that flow into every KB lookup.
    # The Settings dialog / chat header lets the user pin a client etc.
...
class QueryAgent:
    """Chat backend that consults the vault on every turn."""

    def __init__(self, llm: LlmClient, vault_dir: str | Path) -> None:
        self.llm = llm
        self.vault_dir = Path(vault_dir)
        self.reader = SuperMemoryReader(self.vault_dir)
        self._system_prompt = _load_query_prompt(self.vault_dir)
    ...
    def _build_messages(self, session: ChatSession, turn: ChatTurn) -> list[Message]:
        ...
        if ctx is not None and ctx.has_content:
            kb_prompt = SuperMemoryReader.format_for_prompt(ctx)
            if kb_prompt:
                messages.append(system(kb_prompt))
            turn.kb_summary = ctx.summary()
            turn.kb_paths = [
                p
